...
def main(args):
    ...
    ## coco_api 를 사용한 dataset validation = evaluate AP 계산하는 방법
    ...
    ## Model Parameters load 하기(2.1) - Evaluate 하기 위한 파라메터
    output_dir = Path(args.output_dir)
    if args.resume:
        if args.resume.startswith('https'): # 사이트로 시작한다면 
            checkpoint = torch.hub.load_state_dict_from_url(args.resume, map_location='cpu', check_hash=True)
        else:
            checkpoint = torch.load(args.resume, map_location='cpu') # 컴퓨터 내부 저장된 .pth 파일이라면
        model_without_ddp.load_state_dict(checkpoint['model'])

    ## Model Parameters load 하기(2.2) - Train 하기 위한 파라메터
    # (".pth 파일" 내부에 파라미터에 대한 내용 뿐만 아니라, optimizer, lr_scheduler, epoch 의 내용도 있어야 한다 )
    ...
    ## Evaluation만 하고 코드 종료(return) (1) model infernece하기 (2) coco api로 AP 구하기
    if args.eval:
